...
def _find_repo_root() -> Path:
    ...
    for parent in [here.parent, *here.parents]:
        if (parent / "Makefile").exists() and (parent / "app" / "chat" / "backend" / "chat_server.py").exists():
            return parent
    raise RuntimeError("Could not locate repo root (expected Makefile and app/chat/backend/chat_server.py)")
...
def _run_make_target(repo_root: Path, target: str, log_path: Path) -> dict:
    ...
    # Extension builds from extension/ directory, playground from root
    if target == "build-extension":
        command = ["make", "build"]
        cwd = repo_root / "extension"
    else:
        command = ["make", target]
        cwd = repo_root
