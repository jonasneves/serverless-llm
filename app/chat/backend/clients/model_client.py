...
RESTRICTED_MODEL_PATTERNS = ['o1', 'o3', 'o4', 'gpt-5']


def _is_restricted_model(model_id: str) -> bool:
    """Check if model has restricted parameters (no temperature, uses max_completion_tokens)"""
    return any(pattern in model_id.lower() for pattern in RESTRICTED_MODEL_PATTERNS)
...
class ModelClient:
    ...
    def _build_api_payload(self, model_id: str, messages: List[Dict], max_tokens: int,
                           temperature: float, stream: bool = False,
                           response_format: Optional[Dict] = None) -> Dict[str, Any]:
        """Build API request payload with proper parameter handling for restricted models"""
        payload = {"model": model_id, "messages": messages}

        if _is_restricted_model(model_id):
    ...
    async def _parse_sse_stream(self, response, model_id: str) -> AsyncGenerator[Dict[str, Any], None]:
        ...
        yield {"type": "done", "full_content": full_content, "usage": final_usage, "model_id": model_id}
    ...
    async def _call_api_model(self, model_id, messages, max_tokens, temperature, response_format):
        ...
        payload = self._build_api_payload(model_id, messages, max_tokens, temperature,
    ...
    async def _stream_api_model(self, model_id, messages, max_tokens, temperature):
        ...
        payload = self._build_api_payload(model_id, messages, max_tokens, temperature, stream=True)
