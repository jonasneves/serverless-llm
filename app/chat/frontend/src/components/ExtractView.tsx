...
export default function ExtractView({ onClose }: ExtractViewProps) {
  ...
  return (
    <div className="fixed inset-0 z-[300] bg-slate-950/95 backdrop-blur-md flex flex-col text-slate-200">
      {/* Header */}
      <div className="flex items-center justify-between px-4 sm:px-6 py-3 border-b border-white/5">
        <div className="flex items-center gap-2.5">
          ...
          <span className="text-[11px] text-slate-500 hidden sm:inline">live vision → JSON</span>
        </div>
        ...
        <div className="flex items-center gap-1 p-1 rounded-lg bg-black/30 border border-white/5">
          {(Object.keys(VL_MODELS) as VlModelId[]).map(id => (
            <button
              ...
              className={`px-2.5 py-1 text-xs rounded-md transition-colors ${modelId === id ? 'bg-emerald-500/20 text-emerald-300' : 'text-slate-400 hover:text-slate-200'}`}
        ...
        <button
          onClick={onClose}
          className="min-w-[40px] min-h-[40px] w-10 h-10 rounded-full bg-slate-800/60 flex items-center justify-center border border-slate-700/50 hover:border-slate-600 transition-colors active:scale-95"
          title="Close"
        >
      ...
      {!webgpu ? (
        ...
        <div className="flex-1 flex flex-col lg:flex-row min-h-0">
          {/* Camera */}
          <div className="lg:flex-1 relative bg-black flex items-center justify-center min-h-[40vh]">
          ...
          {/* Controls + output */}
          <div className="lg:w-[420px] xl:w-[480px] flex flex-col border-t lg:border-t-0 lg:border-l border-white/5 bg-slate-900/40 min-h-0">
            {/* Fields */}
            <div className="p-4 border-b border-white/5">
              <div className="flex items-center justify-between mb-2">
                <span className="text-xs uppercase tracking-wider text-slate-500">Fields</span>
                <button onClick={addField} className="flex items-center gap-1 text-xs text-emerald-400 hover:text-emerald-300">
                ...
                {fields.map((f, i) => (
                  <div key={i} className="flex items-center gap-1.5">
                    <input
                      ...
                      className="flex-1 px-2.5 py-1.5 text-sm rounded-md bg-black/30 border border-white/10 focus:border-emerald-500/50 focus:outline-none placeholder:text-slate-600"
                    />
                    <button onClick={() => removeField(i)} className="p-1.5 text-slate-600 hover:text-rose-400" title="Remove">
            ...
            <div className="p-4 border-t border-white/5 space-y-2">
              ...
              <div className="flex items-center gap-3">
                <button
                  ...
                  className={`flex-1 flex items-center justify-center gap-2 px-4 py-2.5 rounded-lg font-medium text-sm transition-all active:scale-[0.98] disabled:opacity-50 disabled:cursor-not-allowed ${running ? 'bg-rose-500/20 text-rose-300 hover:bg-rose-500/30' : 'bg-emerald-500/20 text-emerald-300 hover:bg-emerald-500/30'}`}
                >
                  {loadState === 'loading'
